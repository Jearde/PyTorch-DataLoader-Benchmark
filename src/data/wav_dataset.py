...
class WAVDataset(Dataset):
    def __init__(
        ...
        audio_slice_length: float | None = None,
        audio_slice_overlap: float | None = None,
    ...
    def get_audio(self, idx: int) -> Tuple[Tensor, int]:
        ...
        if len(audio[0]) < self.target_length * self.sample_rate:
            audio = torch.nn.functional.pad(
                audio,
                pad=(0, int(self.target_length * self.sample_rate) - len(audio[0])),
                mode="constant",
                value=0,
            )
        ...
        if len(audio[0]) > self.target_length * self.sample_rate:
            audio = audio[:, : int(self.target_length * self.sample_rate)]
